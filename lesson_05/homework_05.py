...
#sorted_d = sorted(car_data.items(), key = itemgetter(-1));
#sorted_d = sorted(car_data.items(), key = lambda item: item[1])
list_of_data = list(car_data.items())
sorted_l = sorted(list_of_data, key = lambda x: x[:][-1][-1])
#print(list_of_data[0][-1][-1])
#print(sorted_l)
